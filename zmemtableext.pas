...
{
to add to zMemTable.pas (loadfromstream method, after 'Open'
for a:= 0 to len-1 do
  case fields[a].DataType of
    ftFloat: TNumericField(Fields[a]).Displayformat := '#,##0.00';
    ftCurrency: TNumericField(Fields[a]).Displayformat := '#,##0.0000';
  end;
  }

                    {
procedure TZMemTableHelper.Subset(ls: array of integer);
var tmp: TZMemTable; ii, max: integer;
begin
  max := FieldCount;
  tmp := TZMemTable.Create(nil);
  tmp.CloneDataFrom(Self);
  try
    Close;
    FieldDefs.Clear;;
    for ii := 0 to high(ls) do
      if (ls[ii] > -1) and (ls[ii] < max) then
      with tmp.Fields[ls[ii]] do
        FieldDefs.Add(FieldName, DataType, Size, Required);
    Open;
    tmp.First;
    while not tmp.eof do
      begin
        Append;
        for ii := 0 to FieldCount -1 do
          Fields[ii].Value := tmp.Fields[ls[ii]].Value;
        Post;
        tmp.Next;
      end;
  finally
